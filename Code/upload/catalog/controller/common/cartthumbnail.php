...
//				$total = $this->currency->format($this->tax->calculate($product['price'], $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity']);
		...
		$useragent=$_SERVER['HTTP_USER_AGENT'];
		if($this->detector->isMobile($useragent)){
			$this->response->setOutput($this->load->view('default/mobile/common/cart_thumbnail.tpl', $data));
		}
		else{
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/common/cart_thumbnail.tpl')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/common/cart_thumbnail.tpl', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/common/cart_thumbnail.tpl', $data));
			}
		}
	}
}