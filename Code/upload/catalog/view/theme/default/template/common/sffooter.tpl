<footer>
  <div class="foot-container">
      <div class="foot-col">
        <h5>帮助</h5>
        <div class="list-unstyled">
          <div class="footer-list-content"><a href="">建议与反馈</a></div>
          <div class="footer-list-content"><a href="">使用帮助</a></div>
          <div class="footer-list-content"><a href="">意见问题</a></div>
        </div>
      </div>
      <div class="foot-col">
        <h5>关注我们</h5>
        <div class="list-unstyled">
          <div class="footer-list-content"><a href="">官方论坛</a></div>
          <div class="footer-list-content"><a href="">新浪微博</a></div>
          <div class="footer-list-content"><a href="">微信公众号</a></div>
        </div>
      </div>
      <div class="foot-col">
        <h5>商务合作</h5>
        <div class="list-unstyled">
          <div class="footer-list-content">phone number</div>
          <div class="footer-list-content">emial</div>
        </div>
      </div>
      <div class="foot-col">
      	<h5>二维码</h5>
      	<div id="QRcode"></div>
      </div>
    <div class="powered"><?php echo $powered; ?></div>
  </div>
</footer>
</body></html>